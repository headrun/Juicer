...
class JbTalks(JuicerSpider):
    ...
    def parse(self, response):
        ...
        if self.cutoff_dt is None:
            ...
            oneweek_diff = datetime.timedelta(days=100)
    ...
    def parse_thread(self,response):
        ...
        for node in nodes:
                ...
                tid = textify(node.select('.//a//@href[contains(string(),"pid")]'))
                ...
                user_url = textify (node.select(".//td[@class='pls']//div[@class='pi']//div[@class='authi']/a/@href"))
                import pdb;pdb.set_trace()
                ...
                text = textify(node.select(".//div[contains(@class,'postmessage')]//td[contains(@id,'postmessage_')]/text()"))
                item.set('text', text)
                print item.data
                #ield item.process()
