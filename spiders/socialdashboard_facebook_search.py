...
def get_results(search_term, customer_name):
    ...
    for page in xrange(1, 1000000):
        ...
        try:
            last_max_id = open('socialdashboard_facebook_max_id', 'r').read()
        except IOError:
            last_max_id = '1'

        if page == 1:
            f = open('socialdashboard_facebook_max_id', 'w')
        ...
        if loop_break:
            if final_data:
                db.update(DB_NAME, 'socialdashborad_dataentires', spec=ids, doc=final_data, upsert=True)
            break
        else:
            if final_data:
                db.update(DB_NAME, 'socialdashborad_dataentires', spec=ids, doc=final_data, upsert=True)
