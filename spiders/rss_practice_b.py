...
class RSS_Discovery(JuicerSpider):
    ...
    def start_requests(self):
        ...
        query = 'select * from feeds where is_crawled=3 and level=0'
    ...
    def parse(self, response):
        ...
        sql = 'UPDATE feeds SET is_crawled=3 where url="%s"' %(blog_url)
        ...
        if len(_entries['entries']) > 2:
            query = "INSERT INTO feed_urls1(url, rss_url) values"
        ...
        for i in urls:
            if '/comments/' in i:
                level_upt = 'UPDATE feeds SET level = 1 where url ="%s"' %(blog_url)
                upt_cursor.execute(level_upt)
