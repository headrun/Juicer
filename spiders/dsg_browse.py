...
def gen_start_urls(name):
    items = lookup_items('dsg_browse', 'got_page:False', limit=100)
    if not items:
        items = [(None, None, 'http://www.dickssportinggoods.com/home/index.jsp')]
...
class DickssportinggoodsBrowseSpider(JuicerSpider):
    ...
    def parse(self, response):
        ...
        yield item.process()
        urls = [ 
            hdoc.select('//a[@class="mainNavLink"]/@href'),
            hdoc.select('//ul[@id="leftNavUL"]//li//a[@class="leftnavlink"]/@href'),
            hdoc.select('//img//parent::a[@class="results"]/@href'),
        ]
        ...
        nodes = hdoc.select('//h2[@class="prodtitle"]//a/@href')
        for node in nodes:
            url = textify(node)
            item = Item(response, HTML)
            item.set('sk', url)
...
SPIDER = DickssportinggoodsBrowseSpider()
