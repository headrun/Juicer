...
def update(_id):
    db = MySQLdb.connect("localhost","root","root","LINKEDIN")
    cursor_upt = db.cursor()
    upt = "UPDATE linkedin_urls SET is_crawled = 1, modified_at = NOW() WHERE pub_id = %s" % (_id)
    cursor_upt.execute(upt)
    try:
        cursor_upt.execute(upt)
        db.commit()
    except:
        log.info("Failed to update-2 for id %s" %_id)
