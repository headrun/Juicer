...
class Bseindia(JuicerSpider):
    ...
    def parse(self, response):
        ...
        out_file = file('/home/headrun/venu/bse_india','w+')
        out_file.write('%s\n' %(time.ctime()))
        out_file.close()
        ...
        for post in posts:
            ...
            a=b=c=d=e=f=''
            if 'open offer' in title.lower() or 'buyback' in title.lower() or 'tender offer' in title.lower():a=True
            ...
            elif 'open offer' in text.lower() or 'buyback' in text.lower() or 'tender offer' in text.lower():d=True
            ...
            cursor = conn.cursor()
            query = "insert into stock(title,contents,type,pdf_link,dt_posted,created_at,modified_at) values(%s,%s,%s, %s, %s, now(), now()) on duplicate key update modified_at=now()"
