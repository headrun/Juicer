...
class VcsDataCompanyTerminalSpider(JuicerSpider):
    name = 'vcsdatacompany_terminal'
    ...
    def parse(self, response):
        ...
        company_name = textify(hdoc.select('//td[contains(text(), "Company Name")]//following-sibling::td/text()')).replace(":", "").replace("Company Name", "") 
        item.set('company_name', company_name)

        address = textify(hdoc.select('//td[contains(text(), "Address")]//following-sibling::td/text()')).replace(":", "").replace("Address", "") 
        item.set('address',address)

        phone = textify(hdoc.select('//td[contains(text(), "Phone No")]//following-sibling::td/text()')).replace(":", "").replace("Phone No", "") 
        item.set('phone', phone)

        website_url = textify(hdoc.select('//td[contains(text(), "Website")]//following-sibling::td/a/text()')).replace(":", "").replace("Website", "") 
