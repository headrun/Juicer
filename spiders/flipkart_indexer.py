...
SETTINGS_FILE = "latest_updated.flipkart_indexer"

MAX_RECORDS_CRAWL = 100
...
def pull_data(num_records):
    ...
    records = db.find("juicerprod", "flipkart_data", spec={"_updated": {"$gt":latest_updated}}, sort=[["_updated", 1]], limit=MAX_RECORDS_CRAWL)
    ...
    alchemyObj.loadAPIKey("b051e282fb7a4acaec8ed41d73b1c37535927a5d");
    ...
    for record in records:
        ...
        if description and len(description) > 5:
            s_result = eval(alchemyObj.TextGetTextSentiment(description, eparams))
    ...
    db.update('juicerprod', 'flipkart_data', spec=db_ids, doc=db_records)
    search.index("flipkart", "item", _records, _ids)
